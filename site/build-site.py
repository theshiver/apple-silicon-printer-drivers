...
<main>
{body}
<footer>Open source (Gutenprint GPL-2.0, brlaser GPL-2.0-or-later, scripts MIT) · <a href="https://github.com/sponsors/theshiver">☕ Buy me a coffee</a> · <a href="{REPO}">Source &amp; issues on GitHub</a> · Not affiliated with Canon, Epson, HP or Apple. Product names are trademarks of their owners.</footer>
</main>
</body></html>"""
...
(DOCS / "brands" / "index.html").write_text(page(
    ...
    f"{len(models)} printer models from {len(BRANDS)} brands supported by native arm64 Gutenprint and brlaser drivers for macOS.",
    f"{BASE}/brands/",
...
function show(m){{res.innerHTML='';q.value=m.name; ans.classList.remove('hidden');
 if(m.unreleased){{ans.innerHTML=`<h3 style="margin-top:0">${{m.name}} support is in the next release</h3><p>The driver is available in the source tree. Existing release installers do not include it yet. See the <a href="{REPO}#brother-hl-1210w">build and setup instructions</a>. Physical printing still needs verification.</p>`;history.replaceState(null,'','#'+encodeURIComponent(m.id));return}}
